...
3. **kitty** - required by CAO for terminal management
...
The provider detects terminal state from the terminal scrollback (ANSI-stripped):
...
`CAO_TERMINAL_ID` is **not** written to `opencode.json`. OpenCode spawns MCP subprocesses that inherit the kitty window's environment, so the terminal ID propagates naturally — the same mechanism Kiro uses.
...
2. If the prompt comes from a tool outside CAO's vocabulary, respond to it manually in the kitty window, or use `--yolo` to disable all restrictions **(DANGEROUS — allows any command including `aws`, `rm`, `curl`)**.
...
- The `opencode` binary isn't on PATH in the kitty window's shell (check `echo $PATH` inside tmux)
