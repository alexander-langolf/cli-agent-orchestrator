...
- Key sequences prefixed with `C-` or `M-` (e.g., `C-d` for Ctrl+D) are sent as kitty key sequences via `send_special_key()`, which kitty interprets as actual key presses
