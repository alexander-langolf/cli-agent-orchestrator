"""Tests for KittyClient._merge_extra_env (issue #248).
...
def test_merge_with_none_is_noop():
    env = {"HOME": "/home/u"}
    KittyClient._merge_extra_env(env, None)
    assert env == {"HOME": "/home/u"}


def test_merge_with_empty_dict_is_noop():
    env = {"HOME": "/home/u"}
    KittyClient._merge_extra_env(env, {})
    assert env == {"HOME": "/home/u"}


def test_merge_adds_user_supplied_keys():
    env = {"HOME": "/home/u"}
    KittyClient._merge_extra_env(env, {"MNEMOSYNE_DIR": "/root/mn", "X": "y"})
...
def test_merge_overrides_inherited_value():
    ...
    KittyClient._merge_extra_env(env, {"AWS_REGION": "us-west-2"})
    assert env["AWS_REGION"] == "us-west-2"


def test_merge_drops_blocked_prefix(caplog):
    env: dict[str, str] = {}
    KittyClient._merge_extra_env(env, {"CLAUDE_SECRET": "x", "OK": "y"})
...
def test_merge_keeps_allowlisted_claude_auth_var():
    env: dict[str, str] = {}
    KittyClient._merge_extra_env(env, {"CLAUDE_CODE_USE_BEDROCK": "1"})
    assert env["CLAUDE_CODE_USE_BEDROCK"] == "1"


def test_merge_drops_value_at_or_above_cap():
    env: dict[str, str] = {}
    KittyClient._merge_extra_env(env, {"BIG": "x" * 2048, "SMALL": "x" * 2047})
    assert "BIG" not in env
    assert env["SMALL"] == "x" * 2047


def test_is_blocked_env_key_classification():
    assert KittyClient._is_blocked_env_key("CLAUDE_SESSION_ID") is True
    assert KittyClient._is_blocked_env_key("CODEX_TOKEN") is True
    assert KittyClient._is_blocked_env_key("__MISE_WATCH") is True
    ...
    assert KittyClient._is_blocked_env_key("CLAUDE_CODE_USE_BEDROCK") is False
    assert KittyClient._is_blocked_env_key("CLAUDE_CODE_SKIP_FOUNDRY_AUTH") is False
    # Unrelated keys aren't blocked.
    assert KittyClient._is_blocked_env_key("AWS_REGION") is False
    assert KittyClient._is_blocked_env_key("MNEMOSYNE_DIR") is False
