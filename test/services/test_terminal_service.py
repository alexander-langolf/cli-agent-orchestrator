...
class TestTerminalServiceWorkingDirectory:
    ...
    def test_get_working_directory_returns_directory_from_tmux_pane(
        ...
        """Test that get_working_directory returns the directory obtained from kitty window."""
        # Arrange
...
class TestSendSpecialKey:
    ...
    def test_send_special_key_propagates_tmux_errors(self, mock_get_metadata, mock_tmux_client):
        ...
        mock_tmux_client.send_special_key.side_effect = Exception("Kitty send error")

        # Act & Assert
        with pytest.raises(Exception, match="Kitty send error"):
