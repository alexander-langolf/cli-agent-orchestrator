...
class TestCopilotCliProviderCommand:
    ...
    @patch("cli_agent_orchestrator.providers.copilot_cli.zellij_client")
    @patch.dict("os.environ", {}, clear=True)
    def test_command_builds_default(self, mock_Zellij, mock_build_mcp, mock_supports_flag):
        mock_supports_flag.return_value = True
        mock_build_mcp.return_value = '{"mcpServers":{"cao-mcp-server":{"command":"x"}}}'
        mock_Zellij.get_pane_working_directory.return_value = "/tmp/project"
    ...
    @patch("cli_agent_orchestrator.providers.copilot_cli.zellij_client")
    @patch.dict("os.environ", {}, clear=True)
    def test_command_does_not_use_model_env(self, mock_Zellij, mock_build_mcp, mock_supports_flag):
        mock_supports_flag.return_value = True
        mock_build_mcp.return_value = '{"mcpServers":{"cao-mcp-server":{"command":"x"}}}'
        mock_Zellij.get_pane_working_directory.return_value = "/tmp/project"
...
class TestCopilotCliProviderModelFlag:
    ...
    @patch("cli_agent_orchestrator.providers.copilot_cli.zellij_client")
    @patch.dict("os.environ", {}, clear=True)
    def test_command_appends_model_when_set(self, mock_Zellij, mock_build_mcp, mock_supports_flag):
        mock_supports_flag.return_value = True
        mock_build_mcp.return_value = '{"mcpServers":{"cao-mcp-server":{"command":"x"}}}'
        mock_Zellij.get_pane_working_directory.return_value = "/tmp/project"
    ...
    @patch("cli_agent_orchestrator.providers.copilot_cli.zellij_client")
    @patch.dict("os.environ", {}, clear=True)
    def test_command_omits_model_when_unset(self, mock_Zellij, mock_build_mcp, mock_supports_flag):
        mock_supports_flag.return_value = True
        mock_build_mcp.return_value = '{"mcpServers":{"cao-mcp-server":{"command":"x"}}}'
        mock_Zellij.get_pane_working_directory.return_value = "/tmp/project"
    ...
    def test_command_omits_model_when_no_agent_profile(
        self, mock_Zellij, mock_build_mcp, mock_supports_flag
    ):
...
class TestCopilotCliProviderTrustPrompts:
    @patch("cli_agent_orchestrator.providers.copilot_cli.time.sleep")
    @patch("cli_agent_orchestrator.providers.copilot_cli.zellij_client")
    def test_accept_trust_prompts_answers_yes_then_returns(self, mock_Zellij, _mock_sleep):
        provider = CopilotCliProvider("test1234", "test-session", "window-0")
        ...
        mock_Zellij.send_special_key.assert_any_call("test-session", "window-0", "y")
...
class TestCopilotCliProviderStatusDetection:
    ...
    def test_get_status_waiting_user_answer(self, mock_Zellij):
        mock_Zellij.get_history.return_value = "confirm folder trust [y/n]"
        provider = CopilotCliProvider("test1234", "test-session", "window-0")
        assert provider.get_status() == TerminalStatus.WAITING_USER_ANSWER

    @patch("cli_agent_orchestrator.providers.copilot_cli.zellij_client")
    def test_get_status_idle_with_no_user_message(self, mock_Zellij):
        mock_Zellij.get_history.return_value = "GitHub Copilot v0.0.415\n❯ Type @ to mention files"
        provider = CopilotCliProvider("test1234", "test-session", "window-0")
        assert provider.get_status() == TerminalStatus.IDLE

    @patch("cli_agent_orchestrator.providers.copilot_cli.zellij_client")
    def test_get_status_idle_with_wrapped_prompt_helper(self, mock_Zellij):
    ...
    @patch("cli_agent_orchestrator.providers.copilot_cli.zellij_client")
    def test_get_status_processing_when_no_idle_prompt(self, mock_Zellij):
        mock_Zellij.get_history.return_value = "Working on edits..."
        provider = CopilotCliProvider("test1234", "test-session", "window-0")
        assert provider.get_status() == TerminalStatus.PROCESSING

    @patch("cli_agent_orchestrator.providers.copilot_cli.zellij_client")
    def test_get_status_error_while_processing_if_error_after_user(self, mock_Zellij):
        mock_Zellij.get_history.return_value = "❯ refactor this\nError: failed to parse"
        provider = CopilotCliProvider("test1234", "test-session", "window-0")
        assert provider.get_status() == TerminalStatus.ERROR

    @patch("cli_agent_orchestrator.providers.copilot_cli.zellij_client")
    def test_get_status_completed_when_response_present_and_idle(self, mock_Zellij):
        mock_Zellij.get_history.return_value = "❯ refactor this\n● Edit file.py (+1 -1)\n❯ "
        provider = CopilotCliProvider("test1234", "test-session", "window-0")
        assert provider.get_status() == TerminalStatus.COMPLETED

    @patch("cli_agent_orchestrator.providers.copilot_cli.zellij_client")
    def test_get_status_processing_when_spinner_visible_with_idle_prompt(self, mock_Zellij):
        mock_Zellij.get_history.return_value = (
    ...
    @patch("cli_agent_orchestrator.providers.copilot_cli.zellij_client")
    def test_get_status_error_when_idle_and_error_without_assistant_marker(self, mock_Zellij):
        mock_Zellij.get_history.return_value = "❯ refactor this\nError: failed to parse\n❯ "
        provider = CopilotCliProvider("test1234", "test-session", "window-0")
        assert provider.get_status() == TerminalStatus.ERROR

    @patch("cli_agent_orchestrator.providers.copilot_cli.zellij_client")
    def test_get_status_completed_when_idle_and_error_with_assistant_marker(self, mock_Zellij):
        mock_Zellij.get_history.return_value = "❯ refactor this\nassistant: note\nError: sample\n❯ "
        provider = CopilotCliProvider("test1234", "test-session", "window-0")
        assert provider.get_status() == TerminalStatus.COMPLETED
...
class TestCopilotCliProviderMisc:
    @patch("cli_agent_orchestrator.providers.copilot_cli.zellij_client")
    def test_send_enter_uses_zellij_client(self, mock_Zellij):
        provider = CopilotCliProvider("test1234", "test-session", "window-0")
