...
| **clients/tmux.py** | ~30% | Requires real kitty sessions for full coverage. Core `send_keys` behavior (literal mode, chunking) is unit-tested via `test_tmux_send_keys.py`. Operations like session creation and history capture are better covered by integration tests. |
| **api/main.py** | 44% | FastAPI endpoints require async testing setup with TestClient and running event loops. Endpoints interact with the database, kitty sessions, and providers simultaneously. Better tested via end-to-end integration tests. |
...
@patch("cli_agent_orchestrator.providers.kiro_cli.KittyClient")
