...
class TestInfoCommand:
    ...
    def test_info_not_in_tmux(self):
        """Test output when not running inside a CAO kitty window."""
        runner = CliRunner()
    ...
    def test_info_in_tmux_non_cao_session(self):
        """Test output when in a terminal but not a CAO session."""
        runner = CliRunner()
        with patch.dict("os.environ", {"CAO_SESSION_NAME": "my-random-session"}, clear=True):
            result = runner.invoke(info)
    ...
    def test_info_in_cao_session_server_responds(self):
        ...
        with patch.dict("os.environ", {"CAO_SESSION_NAME": "cao-test-session"}, clear=True):
